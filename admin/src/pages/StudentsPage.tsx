...
import { Check, ChevronLeft, ChevronRight, Clock3, FileText, LockKeyhole, Printer, Search, ShieldCheck, UserRound, Video } from 'lucide-react';
...
export default function StudentsPage() {
  ...
  useEffect(() => {
    ...
    const request = activeTab === 'exams'
      ? getStudentExamAttempts(routeStudentId, detailPage, 10).then((result) => { setAttempts(result.attempts); setDetailPagination(result.pagination); })
      : getStudentVideoActivity(routeStudentId, detailPage, 10).then((result) => { setVideoActivity(result.activity); setDetailPagination(result.pagination); });
  ...
  }, [routeStudentId, activeTab, detailPage]);
  ...
  return <AppShell><div className="admin-page-stack students-directory-page"><section className="admin-page-intro"><div><span className="admin-eyebrow">متابعة ومساحة الطلاب</span><h1>الطلاب</h1><p>ابحث عن الطالب، افتح ملفه، وتابع المحتوى والامتحانات والفيديوهات من مكان واحد.</p></div>{routeStudentId && <button type="button" className="admin-secondary" onClick={() => window.print()}><Printer size={16} /> طباعة التقرير</button>}</section>{error && <div className="admin-error" role="alert">{error}</div>}<div className="students-directory-layout"><section className="students-list-panel"><div className="admin-section-heading"><div><span className="admin-eyebrow">قائمة الطلاب</span><h2>كل الطلاب</h2></div><span className="admin-count">{pagination?.totalItems || 0}</span></div><div className="admin-student-filters"><div className="admin-search"><Search size={15} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="اسم أو رقم هاتف" /></div><label className="admin-select full-select"><select value={level} onChange={(event) => setLevel(event.target.value)}><option value="">كل الصفوف</option>{levels.map((item) => <option value={item._id} key={item._id}>{item.nameAr}</option>)}</select><ChevronRight size={14} /></label></div>{loading ? <div className="admin-loading-panel small"><span className="spinner" /> جارٍ تحميل الطلاب...</div> : students.length === 0 ? <div className="admin-empty-mini"><UserRound size={21} /><p>لا يوجد طلاب بهذا البحث.</p></div> : <div className="student-list">{students.map((student) => <button type="button" className={`student-list-row ${student._id === routeStudentId ? 'is-selected' : ''}`} key={student._id} onClick={() => selectStudent(student._id)}><span className="student-avatar">{student.name?.charAt(0) || 'ط'}</span><span><strong>{student.name}</strong><small>{student.educationalLevel?.nameAr || 'صف غير محدد'} · {student.phone}</small></span><ChevronLeft size={15} /></button>)}</div>}{pagination && <PaginationControls pagination={pagination} onChange={setPage} />}</section><section className="student-detail-panel">{!routeStudentId ? <div className="admin-empty student-detail-empty"><ShieldCheck size={28} /><h2>اختار طالبًا من القائمة</h2><p>ستظهر هنا بياناته وتفعيل المحتوى ونتائج الامتحانات ونشاط الفيديو.</p></div> : detailLoading && !overview ? <div className="admin-loading-panel"><span className="spinner" /> جارٍ فتح ملف الطالب...</div> : overview && access ? <><StudentProfileHeader overview={overview} onTab={selectTab} activeTab={activeTab} />{activeTab === 'overview' && <OverviewPanel overview={overview} onTab={selectTab} />}{activeTab === 'access' && <AccessPanel access={access} savingKey={savingKey} onToggleCourse={toggleCourse} onToggleLesson={toggleLesson} />}{activeTab === 'exams' && <AttemptsPanel attempts={attempts} pagination={detailPagination} loading={detailLoading} onPage={setDetailPage} />}{activeTab === 'videos' && <VideoActivityPanel activity={videoActivity} pagination={detailPagination} loading={detailLoading} onPage={setDetailPage} />}</> : <div className="admin-empty"><UserRound size={25} /><h2>الطالب غير موجود</h2><button type="button" className="admin-secondary" onClick={() => navigate('/students')}>العودة لقائمة الطلاب</button></div>}</section></div></div></AppShell>;
}
...
function AttemptsPanel({ attempts, pagination, loading, onPage }: { attempts: any[]; pagination: Pagination | null; loading: boolean; onPage: (page: number) => void }) {
  return <section className="student-panel-section"><div className="admin-section-heading"><div><span className="admin-eyebrow">نتائج دقيقة</span><h3>كل محاولات الامتحانات</h3></div><span className="admin-count">{pagination?.totalItems || 0}</span></div>{loading ? <div className="admin-loading-panel small"><span className="spinner" /> جارٍ تحميل المحاولات...</div> : attempts.length === 0 ? <div className="admin-empty-mini">لا توجد محاولات لهذا الطالب.</div> : <div className="student-data-table"><div className="student-data-head"><span>الامتحان</span><span>المحاولة</span><span>الدرجة</span><span>التاريخ</span></div>{attempts.map((attempt) => <div className="student-data-row" key={attempt._id}><span><strong>{attempt.examId?.title || 'امتحان'}</strong><small>{attempt.submittedReason === 'timeout' ? 'انتهى الوقت' : 'تسليم يدوي'}</small></span><span>{attempt.attemptNumber || 1}</span><b className={attempt.isPassed ? 'is-good' : 'is-low'}>{attempt.score} / {attempt.totalMarks} · {attempt.percentage}٪</b><span>{new Date(attempt.submittedAt).toLocaleDateString('ar-EG')}</span></div>)}</div>}{pagination && <PaginationControls pagination={pagination} onChange={onPage} />}</section>;
}

function VideoActivityPanel({ activity, pagination, loading, onPage }: { activity: any[]; pagination: Pagination | null; loading: boolean; onPage: (page: number) => void }) {
  return <section className="student-panel-section"><div className="admin-section-heading"><div><span className="admin-eyebrow">متابعة الفيديو</span><h3>نشاط المشاهدة</h3></div><span className="admin-count">{pagination?.totalItems || 0}</span></div>{loading ? <div className="admin-loading-panel small"><span className="spinner" /> جارٍ تحميل النشاط...</div> : activity.length === 0 ? <div className="admin-empty-mini">لم يبدأ الطالب أي فيديو بعد.</div> : <div className="student-data-table"><div className="student-data-head"><span>الدرس</span><span>الجلسات</span><span>المشاهدة</span><span>آخر نشاط</span></div>{activity.map((item) => <div className="student-data-row" key={item._id}><span><strong>{item.lessonId?.title || 'درس'}</strong><small>{item.lessonId?.courseId?.title || 'باب غير معروف'}</small></span><span>{item.sessionCount || 0} مرات</span><b>{Math.round((item.watchedSeconds || 0) / 60)} دقيقة · {Math.round(item.completionPercent || 0)}٪</b><span>{new Date(item.lastWatchedAt).toLocaleDateString('ar-EG')}</span></div>)}</div>}{pagination && <PaginationControls pagination={pagination} onChange={onPage} />}</section>;
}
