...
export const getStudentExamAttempts = async (userId: string, page = 1, limit = 10) => { const response = await api.get<{ data: any[]; pagination: Pagination }>(`/admin/users/${userId}/exam-attempts`, { params: { page, limit } }); return { attempts: response.data.data, pagination: response.data.pagination }; };
export const getStudentVideoActivity = async (userId: string, page = 1, limit = 10) => { const response = await api.get<{ data: any[]; pagination: Pagination }>(`/admin/users/${userId}/video-activity`, { params: { page, limit } }); return { activity: response.data.data, pagination: response.data.pagination }; };
