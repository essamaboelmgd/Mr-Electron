...
export default function CourseDetailPage() {
  ...
  const [videoSource, setVideoSource] = useState<{ provider: 'youtube' | 'vimeo' | 'bunny'; videoId: string; videoStatus?: string } | null>(null);
  ...
  const load = () => {
    ...
    Promise.all([getCourseById(id), getCourseLessons(id), getCourseExams(id)]).then(([chapter, chapterLessons, chapterExams]) => { setCourse(chapter); setLessons(chapterLessons); setExams(chapterExams); }).catch((requestError: unknown) => setError(requestMessage(requestError, 'تعذر تحميل الباب.'))).finally(() => setLoading(false));
  };
  useEffect(load, [id]);
