...
export default function ExamReviewPage() {
  ...
  useEffect(() => { if (!id) return; Promise.all([getExamById(id), getExamResults(id), getExamQuestions(id, true)]).then(([examData, resultData, questionData]) => { setExam(examData); setResult(resultData); setQuestions(questionData); }).finally(() => setLoading(false)); }, [id]);
  if (loading) return <AppShell><div className="loading-panel"><span className="spinner" /> جارٍ تجهيز المراجعة...</div></AppShell>;
  if (!exam || !result) return <AppShell><div className="error-state"><h1>المراجعة غير متاحة</h1><button type="button" className="outline-button" onClick={() => navigate('/exams')}>العودة</button></div></AppShell>;
