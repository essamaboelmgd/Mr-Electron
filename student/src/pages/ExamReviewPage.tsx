...
export default function ExamReviewPage() {
  ...
  useEffect(() => { if (!id) return; const attemptId = searchParams.get('attemptId') || undefined; setLoading(true); setError(''); Promise.all([getExamById(id), getExamResults(id, attemptId), getExamQuestions(id, true, attemptId)]).then(([examData, resultData, questionData]) => { setExam(examData); setResult(resultData); setQuestions(questionData); }).catch((requestError: unknown) => { const typed = requestError as { response?: { data?: { message?: string } } }; setError(typed.response?.data?.message || 'المراجعة مغلقة حاليًا أو لم يحدد المدرس موعد فتحها.'); }).finally(() => setLoading(false)); }, [id, searchParams]);
  if (loading) return <AppShell><div className="loading-panel"><span className="spinner" /> جارٍ تجهيز المراجعة...</div></AppShell>;
  if (error || !exam || !result) return <AppShell><div className="error-state"><h1>المراجعة غير متاحة</h1><p>{error || 'المراجعة مغلقة حاليًا أو لم يحدد المدرس موعد فتحها.'}</p><button type="button" className="outline-button" onClick={() => navigate('/exams')}>العودة</button></div></AppShell>;
