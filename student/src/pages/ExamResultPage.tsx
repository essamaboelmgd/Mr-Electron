...
export default function ExamResultPage() {
  ...
  const canRetake = attempts.length < maxAttempts;
  const canReview = Boolean(result.policy?.reviewAvailable);
  return <AppShell><div className="result-page page-stack"><button className="back-link" type="button" onClick={() => navigate('/exams')}><ArrowRight size={17} /> العودة للامتحانات</button><section className={`result-card ${passed ? 'is-passed' : 'is-failed'}`}><span className="result-seal">{passed ? <CheckCircle2 size={32} /> : <XCircle size={32} />}</span><span className="eyebrow">نتيجة {exam.type === 'general' ? 'الامتحان العام' : 'امتحان الباب'} · المحاولة {result.submission.attemptNumber || 1}</span><h1>{passed ? 'نتيجة ممتازة' : 'محاولة جيدة، نراجع ونحاول مرة أخرى'}</h1><p>{exam.title}</p><div className="score-display"><strong>{Math.round(result.percentage)}٪</strong><span>{result.submission.score} من {result.submission.totalMarks} درجات</span></div><div className="result-actions">{canReview ? <button type="button" className="primary-button" onClick={() => navigate(`/exams/${id}/review?attemptId=${result.submission._id}`)}><FileText size={16} /> مراجعة الإجابات</button> : <span className="result-review-closed">المراجعة مغلقة حاليًا من المدرس</span>}{canRetake && <button type="button" className="outline-button" onClick={() => navigate(`/exams/${id}/take`)}><RotateCcw size={16} /> إعادة المحاولة</button>}</div></section>{attempts.length > 1 && <section className="attempt-history"><div className="section-heading compact"><div><span className="eyebrow">سجل المحاولات</span><h2>نتائجك السابقة</h2></div><span className="count-chip">{attempts.length} محاولات</span></div><div className="attempt-history-list">{attempts.map((attempt: any, index) => <button type="button" className={`attempt-history-row ${attempt._id === result.submission._id ? 'is-current' : ''}`} key={attempt._id} onClick={() => navigate(`/exams/${id}/result?attemptId=${attempt._id}`)}><span>المحاولة {attempt.attemptNumber || attempts.length - index}</span><strong>{Math.round(attempt.percentage ?? (attempt.totalMarks ? (attempt.score / attempt.totalMarks) * 100 : 0))}٪</strong><small>{new Date(attempt.submittedAt).toLocaleDateString('ar-EG')}</small></button>)}</div></section>}</div></AppShell>;
}
